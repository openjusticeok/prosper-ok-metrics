...
reporting_period_start <- ymd("2022-01-01")
reporting_period_end <- rollback(ymd("2024-07-01"))
...
all_cases <- ojo_tbl("case", .con = ojodb) |>
  filter(
    case_type == "CM",
    date_filed >= reporting_period_start,
...
dps_violation_persons <- ojo_tbl("party", .con = ojodb) |>
  ...
  mutate(
    ...
    fiscal_quarter = case_when(
      quarter(date_filed) == 1 ~ 3,
      quarter(date_filed) == 2 ~ 4,
      quarter(date_filed) == 3 ~ 1,
      quarter(date_filed) == 4 ~ 2,
      TRUE ~ NA_real_
    )
  ) |>
  distinct(oscn_id, .keep_all = TRUE) |>
  group_by(fiscal_year, fiscal_quarter) |>
  count(name = "lower_bound") |>
  ungroup()
...
both_methods_persons |>
  ...
  distinct(oscn_id, .keep_all = TRUE) |>
  group_by(fiscal_year, fiscal_quarter) |>
  count(name = "upper_bound") |>
  ungroup() |>
  left_join(
    dps_violation_persons,
    by = c("fiscal_year", "fiscal_quarter")
  ) |>
  select(
    fiscal_year,
    fiscal_quarter,
  ...
  tab_header(
    ...
    subtitle = glue::glue("{reporting_period_start} to {reporting_period_end}")) |>
  tab_style(
  ...
  fmt_integer(columns = c(-"fiscal_year")) |>
  gt_theme_nytimes()
```
