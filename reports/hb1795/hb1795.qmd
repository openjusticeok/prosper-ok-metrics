...
abstract: Between _9215_ and _16144_ Oklahomans with misdemeanor drug charges were impacted by provisions in HB 1795 between January 1, 2022 and December 1, 2023.
---
...
# start by looking at the number of individuals with these charges month by month
# unsure why the monthly counts are so different for 2022...
both_methods_persons |> 
  mutate(month_year = floor_date(as_date(created_at), unit = "month")
  ) |> 
  count(month_year)

both_methods_persons_big |> 
  mutate(month_year = 
           floor_date(as_date(created_at), unit = "month")
         ) |> 
  count(month_year)

# option 1) Just use Dec 2022 -- kind of dumb but the easiest route

# option 2) simple moving avg or median -- might be sufficient but not great 

# option 3) weighted moving avg or med.

# option 4) predicting Dec. 2023 using something more high brow like KNN -- might be overkill 
