...
test_that("placeholder time series is deterministic and well-formed", {
  ...
  expect_identical(names(ts), c("Time", "Good Stuff"))
  expect_equal(nrow(ts), 7) # 7 time points
  expect_equal(sum(diff(ts$`Good Stuff`) < 0), 2) # 2 kinks downward
})
...
test_that("placeholder_gt returns a gt table with placeholder data", {
  ...
  data <- tbl[["_data"]]
  expect_identical(names(data), c("Time", "Good Stuff"))
...
test_that("placeholder_highchart returns a line chart", {
  ...
  expect_equal(first_point$y, .placeholder_time_series()$`Good Stuff`[1])
})
